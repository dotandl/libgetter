...
TEST(BoxInfo, CreatesFromJSONAllFields) {
  ...
  EXPECT_STREQ(bi->authors->data[0], "Josh");
  EXPECT_STREQ(bi->authors->data[1], "Jimmy");
  EXPECT_STREQ(bi->categories->data[0], "Utilities");
  ...
  EXPECT_STREQ(bi->dependencies->data[0], "box1");
  EXPECT_STREQ(bi->dependencies->data[1], "box2");
  EXPECT_STREQ(bi->build_dependencies->data[0], "box3");
  EXPECT_STREQ(bi->build_dependencies->data[1], "box4");
  EXPECT_STREQ(bi->build_dependencies->data[2], "box5");
  EXPECT_STREQ(bi->optional_dependencies->data[0], "box6");
  EXPECT_STREQ(bi->conflicts->data[0], "box7");
  EXPECT_STREQ(bi->conflicts->data[1], "box8");
  EXPECT_STREQ(bi->replaces->data[0], "box9");
  EXPECT_STREQ(bi->replaces->data[1], "box10");
...
TEST(BoxInfo, CreatesFromJSONRequiredFields) {
  ...
  EXPECT_STREQ(bi->authors->data[0], "Jack");
